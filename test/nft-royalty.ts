...
describe("Test NFT royalty", async function () {
    it("royaltyInfo calculation", async function () {
        ...
        const { nft } = await Collections.mintNFTWithRoyalty(
            collection,
            owner,
            {
                royalty: ROAYLTY,
        ...
        const actual = await nft.methods.royaltyInfo({ answerId: 0, salePrice: SALE_PRICE }).call();
